    ...
    P: NextFrameProvider + OriginAdvancer + OriginProvider + SignalReceiver + Debug,
{
    ...
    pub fn prune(&mut self) -> PipelineResult<()> {
        ...
        let max_channel_bank_size = FJORD_MAX_CHANNEL_BANK_SIZE;
...
mod tests {
    ...
    fn test_ingest_invalid_frame() {
        ...
        let mock = TestNextFrameProvider::new(vec![]);
        let mut channel_bank = ChannelBank::new(Arc::new(SoonRollupConfig::default()), mock);
        ...
        assert_eq!(channel_bank.size(), FRAME_OVERHEAD);
