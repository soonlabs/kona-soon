    ...
    P: NextBatchProvider + OriginAdvancer + OriginProvider + SignalReceiver + Send + Debug,
{
    async fn next_batch(&mut self, parent: L2BlockInfo) -> PipelineResult<SingleBatch> {
        ...
        info!(
            target: "batch-validator",
            "Received SingleBatch: parent_hash={}, epoch_num={}, epoch_hash={}, timestamp={}, tx_count={}",
            next_batch.parent_hash,
            next_batch.epoch_num,
            next_batch.epoch_hash,
            next_batch.timestamp,
            next_batch.transactions.len()
        );
...
mod test {
    ...
    async fn test_batch_validator_next_batch_valid() {
        let cfg = Arc::new(SoonRollupConfig {
            block_time: 2,
    ...
    async fn test_batch_validator_next_batch_sequence_window_expired() {
        ...
        let cfg = Arc::new(SoonRollupConfig { seq_window_size: 5, ..Default::default() });
        let mut mock = TestBatchQueueProvider::new(vec![]);
        mock.origin = Some(BlockInfo { number: 1, ..Default::default() });
        let mut bv = BatchValidator::new(cfg, mock);
