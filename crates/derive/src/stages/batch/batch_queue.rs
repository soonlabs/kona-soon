    ...
    BF: L2ChainProvider + Debug,
{
    ...
    pub fn new(cfg: Arc<SoonRollupConfig>, prev: P, fetcher: BF) -> Self {
        Self {
            cfg,
            prev,
            origin: None,
    ...
    pub async fn derive_next_batch(
        ...
        if next_timestamp < next_epoch.timestamp || first_of_epoch {
            ...
            info!(
                target: "batch-queue",
                "Generated empty SingleBatch: parent_hash={}, epoch_num={}, epoch_hash={}, timestamp={}, tx_count={}",
                empty_batch.parent_hash,
                empty_batch.epoch_num,
                empty_batch.epoch_hash,
                empty_batch.timestamp,
                empty_batch.transactions.len()
            );
            return Ok(Batch::Single(empty_batch));
        }
    ...
    pub async fn add_batch(&mut self, batch: Batch, parent: L2BlockInfo) -> PipelineResult<()> {
        ...
        let drop = validity.is_drop() || validity.is_future();
        if drop {
            self.prev.flush();
            return Ok(());
...
mod tests {
    ...
    use soon_primitives::da::channel::FJORD_MAX_RLP_BYTES_PER_CHANNEL;
    use tracing::Level;
    ...
    fn new_batch_reader() -> BatchReader {
        ...
        BatchReader::new(bytes, FJORD_MAX_RLP_BYTES_PER_CHANNEL as usize)
    }
    ...
    async fn test_holocene_add_batch_valid() {
        ...
        let cfg = Arc::new(SoonRollupConfig { max_sequencer_drift: 700, ..Default::default() });
    ...
    async fn test_add_old_batch_drop_holocene() {
        ...
        let cfg = Arc::new(SoonRollupConfig { block_time: 2, ..Default::default() });
        let batch = SingleBatch {
            parent_hash: B256::default(),
            epoch_num: 0,
            epoch_hash: B256::default(),
    ...
    async fn test_next_batch_clear_next_spans() {
        let mut reader = new_batch_reader();
        let cfg = Arc::new(SoonRollupConfig { block_time: 100, ..Default::default() });
        let mut batch_vec: Vec<PipelineResult<Batch>> = vec![];
        while let Some(batch) = reader.next_batch(cfg.as_ref()) {
            batch_vec.push(Ok(batch));
        }
