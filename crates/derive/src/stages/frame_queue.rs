    ...
    P: FrameQueueProvider + OriginAdvancer + OriginProvider + SignalReceiver + Debug,
{
    ...
    /// The rollup config.
    #[allow(dead_code)]
    pub rollup_config: Arc<SoonRollupConfig>,
}
    ...
    P: FrameQueueProvider + OriginAdvancer + OriginProvider + SignalReceiver + Debug,
{
    ...
    pub fn is_holocene_active(&self, _origin: BlockInfo) -> bool {
        false
    }
    ...
    pub async fn load_frames(&mut self) -> PipelineResult<()> {
        ...
        let frames = match Frame::parse_frames(&data.into()) {
            ...
            Err(err) => {
                ...
                error!(target: "frame-queue", "Failed to parse frames from data, err:{}", err);
                return Ok(());
            }
        };
...
pub(crate) mod tests {
    ...
    async fn test_holocene_valid_frames() {
        ...
        let assert = crate::test_utils::FrameQueueBuilder::new()
            .with_rollup_config(&SoonRollupConfig::default())
            .with_origin(BlockInfo::default())
            .with_expected_frames(&frames)
            .with_frames(&frames)
            .build();
    ...
    async fn test_holocene_non_sequential_frames() {
        ...
        let assert = crate::test_utils::FrameQueueBuilder::new()
            .with_rollup_config(&SoonRollupConfig {
