...
impl SingleBatch {
    ...
    pub fn check_batch(
        ...
        if self.timestamp < l2_safe_head.block_info.timestamp {
            warn!(
                "dropping batch with old timestamp, min_timestamp: {}",
        ...
        if self.epoch_num < epoch.number {
            ...
            // Batch is sticking to the current epoch, or parent block number is 0, continue.
            ...
            if l1_blocks.len() < 2 {
                info!(
                    "eager batch wants to advance epoch, but could not without more L1 blocks at epoch: {:?}",
                    epoch.id()
                );
                return BatchValidity::Undecided;
            }
        ...
        if self.epoch_hash != batch_origin.hash && l2_safe_head.block_info.number != 0 {
        ...
        let max_drift = cfg.max_sequencer_drift;
        let max = if let Some(max) = batch_origin.timestamp.checked_add(max_drift) {
            max
        } else {
            warn!(
                "dropped batch, timestamp exceeds configured max sequencer drift, origin timestamp: {}, max drift: {}",
                batch_origin.timestamp, max_drift
            );
            return BatchValidity::Drop;
        };
        ...
        if self.timestamp > max && !no_txs {
            ...
            warn!(
                "batch exceeded sequencer time drift, sequencer must adopt new L1 origin to include transactions again, max time: {max}"
            );
            return BatchValidity::Drop;
        }
        if self.timestamp > max && no_txs {
            ...
            if epoch.number == batch_origin.number {
                ...
                if self.timestamp >= next_origin.timestamp {
                    info!(
                        "empty batch that exceeds the time drift without adopting next L1 origin, dropping"
                    );
                    return BatchValidity::Drop;
                } else {
...
mod tests {
    ...
    fn test_drop_prior_to_inclusion_block() {
        ...
        let cfg = SoonRollupConfig { seq_window_size: 44, ..Default::default() };
        let l1_blocks = vec![BlockInfo::default()];
        let l2_safe_head = L2BlockInfo::default();
    ...
    #[test]
    #[ignore] // remove batch time check
    fn test_drop_old_batch() {
        ...
        let cfg = SoonRollupConfig { block_time: 2, ..Default::default() };
        let l1_blocks = vec![BlockInfo::default()];
        let l2_safe_head = L2BlockInfo::default();
    ...
    fn test_drop_prior_epoch_num() {
        ...
        let cfg = SoonRollupConfig { seq_window_size: 100, ..Default::default() };
    ...
    #[test]
    fn test_undecided_epoch_num_plus_one_too_few_l1_blocks() {
        let single_batch = SingleBatch {
            parent_hash: B256::ZERO,
            epoch_num: 0xFF,
            epoch_hash: B256::ZERO,
        ...
        let cfg = SoonRollupConfig { seq_window_size: 100, ..Default::default() };
        let l1_blocks = vec![BlockInfo { number: 254, ..Default::default() }];
        let mut l2_safe_head = L2BlockInfo::default();
    ...
    #[test]
    fn test_drop_epoch_num_plus_one_bad_hash() {
        let single_batch = SingleBatch {
            parent_hash: B256::ZERO,
            epoch_num: 0xFF,
            epoch_hash: B256::ZERO,
        ...
        let cfg = SoonRollupConfig { seq_window_size: 100, ..Default::default() };
        let l1_blocks = vec![
            BlockInfo { number: 254, ..Default::default() },
            BlockInfo { number: 255, hash: B256::from([1u8; 32]), ..Default::default() },
        ];
        let mut l2_safe_head = L2BlockInfo::default();
        l2_safe_head.block_info.number = 1;
        let inclusion_block = BlockInfo { number: 300, ..Default::default() };
    ...
    fn test_drop_later_epoch_number() {
        ...
        let cfg = SoonRollupConfig { seq_window_size: 200, ..Default::default() };
