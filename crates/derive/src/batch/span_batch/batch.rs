...
impl SpanBatch {
    ...
    pub async fn check_batch<BF: L2ChainProvider>(
        ...
        // let deposit_count: usize = safe_block_payload
        //     .transactions
        //     .iter()
        //     //TODO check if tx is deposit transaction
        //     //.map(|tx| if tx.is_deposit() { 1 } else { 0 })
        //     .sum();
        // if safe_block_payload.transactions.len() - deposit_count != batch_txs.len() {
        //     warn!(
        //         "overlapped block's tx count does not match, safe_block_txs: {}, batch_txs: {}",
        //         safe_block_payload.transactions.len(),
        //         batch_txs.len()
        //     );
        //     return BatchValidity::Drop;
        // }
        ...
        //     let mut buf = Vec::new();
        //     safe_block_payload.transactions[j + deposit_count].encode_2718(&mut buf);
...
mod tests {
    ...
    async fn test_check_batch_missing_l1_block_input() {
        ...
        let cfg = SoonRollupConfig::default();
        let l1_blocks = vec![];
    ...
    async fn test_check_batch_out_of_order() {
        ...
        let cfg = SoonRollupConfig { block_time: 10, ..Default::default() };
    ...
    async fn test_starting_epoch_too_far_ahead() {
        ...
        let cfg = SoonRollupConfig { seq_window_size: 100, block_time: 10, ..Default::default() };
        let block = BlockInfo { number: 10, timestamp: 10, ..Default::default() };
        let l1_blocks = vec![block];
        let parent_hash = b256!("1111111111111111111111111111111111111111000000000000000000000000");
        let l2_safe_head = L2BlockInfo {
    ...
    #[tokio::test]
    #[ignore] //ignore span batch test case
