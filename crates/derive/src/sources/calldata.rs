...
impl<CP: ChainProvider + Send> CalldataSource<CP> {
    ...
    async fn load_calldata(
        ...
        let txs = self.chain_provider.get_block_transactions_by_hash(block_ref.hash).await?;
        ...
        if !self.calldata.is_empty() {
            info!("l1 block height find data: {}", block_ref.number);
        }
...
mod tests {
    ...
    use alloy_consensus::{Signed, TxEip2930, TxEip4844, TxEip4844Variant, TxEnvelope, TxLegacy};
    ...
    async fn test_load_calldata_wrong_batch_inbox_address() {
        ...
        source
            .chain_provider
            .insert_block_with_transactions(0, block_info, vec![tx]);
        assert!(!source.open); // Source is not open by default.
