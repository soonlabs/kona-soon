    ...
    BF: L2ChainProvider + Debug,
{
    ...
    pub const fn new(prev: P, config: Arc<SoonRollupConfig>, fetcher: BF) -> Self {
...
mod test {
    ...
    async fn test_batch_stream_flush() {
        let config = Arc::new(SoonRollupConfig::default());
    ...
    async fn test_span_buffer() {
        ...
        let config = Arc::new(SoonRollupConfig { block_time: 2, ..SoonRollupConfig::default() });
