    ...
    P: NextFrameProvider + OriginAdvancer + OriginProvider + SignalReceiver + Debug,
{
    ...
    pub fn new(cfg: Arc<SoonRollupConfig>, prev: P) -> Self {
    ...
    pub fn is_timed_out(&self) -> PipelineResult<bool> {
        ...
        let is_timed_out = self
            .channel
            .as_ref()
            .map(|c| c.open_block_number() + self.cfg.channel_timeout < origin.number)
            .unwrap_or_default();
    ...
    P: NextFrameProvider + OriginAdvancer + OriginProvider + SignalReceiver + Send + Debug,
{
    async fn next_data(&mut self) -> PipelineResult<Option<Bytes>> {
        ...
        if let Some(channel) = self.channel.as_mut() {
            ...
            let max_rlp_bytes_per_channel = self.cfg.channel_size;
            if channel.size() > max_rlp_bytes_per_channel {
                warn!(
                    target: "channel-assembler",
...
mod test {
    ...
    async fn test_assembler_channel_timeout() {
        ...
        let cfg = Arc::new(SoonRollupConfig { channel_size: 10_000_000, ..Default::default() });
        ...
        assembler.prev.block_info =
            Some(BlockInfo { number: assembler.cfg.channel_timeout + 1, ..Default::default() });
