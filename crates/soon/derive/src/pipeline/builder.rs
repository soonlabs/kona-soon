...
pub type BatchProviderStage<DAP, P, T> = BatchQueue<BatchStreamStage<DAP, P, T>, T>;
    ...
    D: DataAvailabilityProvider + Send + Sync + Debug,
{
    ...
    rollup_config: Option<Arc<SoonRollupConfig>>,
    ...
    D: DataAvailabilityProvider + Send + Sync + Debug,
{
    ...
    pub fn rollup_config(mut self, rollup_config: Arc<SoonRollupConfig>) -> Self {
    ...
    D: DataAvailabilityProvider + Send + Sync + Debug,
{
    fn from(builder: PipelineBuilder<B, P, T, D>) -> Self {
        ...
        let mut l1_traversal = L1Traversal::new(
            chain_provider,
            Arc::clone(&rollup_config),
            builder.system_config.unwrap_or_default(),
        );
