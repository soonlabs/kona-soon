...
pub fn decompress_to_vec_zlib(input: &[u8]) -> Result<(Vec<u8>, usize), DecompressError> {
    decompress_to_vec_inner(input, inflate_flags::TINFL_FLAG_PARSE_ZLIB_HEADER, usize::MAX)
}
...
mod test {
    use super::{
        DecompressError, TINFLStatus, decompress_slice_iter_to_slice, decompress_to_vec_zlib,
    ...
    fn decompress_vec() {
        ...
        assert_eq!(res.0.as_slice(), &b"Hello, zlib!"[..]);
    }
