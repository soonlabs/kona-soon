...
fn compress_to_vec_inner(mut input: &[u8], level: u8, window_bits: i32, strategy: i32) -> Vec<u8> {
    ...
    loop {
        let (status, bytes_in, bytes_out) =
            compress(&mut compressor, input, &mut output[out_pos..], TDEFLFlush::Finish);
        out_pos += bytes_out;
...
#[cfg(test)]
mod test {
    use super::{CompressionStrategy, compress_to_vec, compress_to_vec_inner};
    ...
    fn compress_huff_only() {
        ...
        assert_eq!(test_data, d.0.as_slice());
    }
