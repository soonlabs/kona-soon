...
pub enum L2ChainProviderError {
    ...
    #[error("fetch slot block time failed: {0}")]
    GetSlotBlockTimeFailed(String),
    #[error("fetch slot bank hash failed: {0}")]
    GetSlotBankHashFailed(String),
}
...
impl From<L2ChainProviderError> for PipelineErrorKind {
    fn from(e: L2ChainProviderError) -> Self {
        match e {
            ...
            L2ChainProviderError::GetSlotBlockTimeFailed(e) => {
                PipelineErrorKind::Temporary(PipelineError::Provider(e.to_string()))
            }
            L2ChainProviderError::GetSlotBankHashFailed(e) => {
                PipelineErrorKind::Temporary(PipelineError::Provider(e.to_string()))
            }
        }
    }
